...
if Sopt.flag || isfield(Sopt,'init')&&Sopt.init.flag
    ...
    %% FSOLVE CALL 1 %%
    [XP,~,iEx,output,J] = solverfn(@(X) extended_residual(X, ...
    ...
    %% FSOLVE CALL 2 %%
    [XP,~,iEx,output,J] = solverfn(@(X) extended_residual([X;lam_s], ...
...
while istep<=Sopt.stepmax
    ...
    if Sopt.flag
        ...
        [Xtmp,Rext,iEx,output,Jtmp] = ...
            solverfn(@(X) extended_residual( ...
                [X;XP(end)],Xref,zref,fun_residual,Sopt),XP(1:end-1), ...
                   Solopt);
        %%               %%
    end
    %% ACCEPT/REJECT POINT
    if iEx~=1 && norm(Rext(1:end))>=Sopt.eps
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Extended Residual Function Definition
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Finite difference Jacobian function definition
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Nonlinear Solver Definition
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [X,FVAL,EXITFLAG,OUTPUT,JACOB] = ndcsisolve(ext_res, X0, Solopt)
%========================================================================
% DESCRIPTION:
% Function for the solver implemented for the benchmarking done for
% the NDCSI 2018 at Imperial College London in the July of
% 2018. Outputs matched to fsolve outputs (matlab R2018a)
%========================================================================
%
% Called functions:
%   ext_res (function handle)
%   [R, dRdX] = ext_res(X)    
%
% Input data dictionary:
%   ext_res         FunctionHandle  Function handle for the
%                                   extended residual function
%   x0              Rarray          Initial guess vector
%   Solopt.MaxIter  Struct.Rscalar  Maximum number of iterations
%   Solopt.TolFun   Struct.Rscalar  Solver function tolerance for
%                                   iteration termination 
%   Solopt.TolX     Struct.Rscalar  Solver function
%                                            tolerance for
%                                            iteration termination    
%   Solopt.eps		     Struct.Rscalar  Solver tolerance to
%   					     terminate iterations
%
% Output data dictionary:
%   X                   Rarray          Computed solution
%   FVAL                Rarray          Computed function value at
%                                       X 
%   EXITFLAG            Iscalar         EXITFLAG desccribing the
%                                       exit conditon:
%                                         1  FSOLVE converged to a
%                                            root.
%                                         2  Change in X too
%                                            small.
%                                         3  Change in residual
%                                            norm too small.
%                                         4  Computed search
%                                            direction too small.
%                                         0  Too many function
%                                            evaluations or
%                                            iterations.
%                                        -1  Stopped by output/plot
%                                            function.
%                                        -2  Converged to a point
%                                            that is not a root.
%                                        -3  Trust region radius
%                                            too small
%                                            (Trust-region-dogleg).
%   OUTPUT.iterations   Struct.Iscalar Number of iterations
%   OUTPUT.funcCount    Struct.Iscalar Number of function
%                                      evaluations
%   JACOB               Rarray         Jacobian of the function at
%                                      X
    
    [FVAL0, JACOB] = ext_res(X0); % Initial Solution
    Y = zeros(size(X0));
    X = X0;
    OUTPUT.iterations = 0;
    OUTPUT.funcCount = 1;
    FOO = max(max(JACOB));
    if strcmp(Solopt.Display,'iter')==1
        fprintf(['Iteration\t Func-count\t f(x)\t Step norm\t F-' ...
                 'OOpt\n']);
        fprintf('%d\t %d\t %f\t %f\t %d\n', OUTPUT.iterations,...
            OUTPUT.funcCount, norm(FVAL0), norm(X-X0), FOO);
    end    
    while 1
        % Update
        [L,U,p] = lu(JACOB,'vector');
        X = X0 - U\(L\(FVAL0(p,1)));
        OUTPUT.iterations = OUTPUT.iterations + 1;
        
        % Evaluate Function
        [FVAL, JACOB] = ext_res(X);
        ...
        if norm(FVAL)<Solopt.TolFun
            EXITFLAG = 1; % fsolve converged to a root
            break;
        end
        if norm(X-X0)<Solopt.TolX
            EXITFLAG = 2; % Change in X too small
            break;
        end
        if norm(FVAL-FVAL0)<Solopt.TolFun
            EXITFLAG = 3; % Change in residual norm too small
            break;
        end
        if OUTPUT.iterations > Solopt.MaxIter
    ...
    if EXITFLAG~=0 && norm(FVAL)>Solopt.TolFun
        EXITFLAG = -2; % Converged to a point that's not a root
    end
end
